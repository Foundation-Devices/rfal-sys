...
void st25r95UartRxCpltCallback(void)
{
    ...
    switch (st25r95UARTRxCtx.RxState)
    {
        case ST25R95_CB_RESLEN: /*  Result and Len received */
            ...
            st25r95UARTRxCtx.retCode = RFAL_ERR_NONE;
            ...
            switch (result)
            {
                ...
                case ST25R95_ERRCODE_COMERROR:
                    st25r95UARTRxCtx.retCode = RFAL_ERR_INTERNAL;
                    break;
                case ST25R95_ERRCODE_FRAMEWAITTIMEOUT:
                    st25r95UARTRxCtx.retCode = RFAL_ERR_TIMEOUT;
                    break;
                case ST25R95_ERRCODE_OVERFLOW:
                    st25r95UARTRxCtx.retCode = RFAL_ERR_HW_OVERRUN;
                    break;
                ...
                case ST25R95_ERRCODE_68_TR1TOOSHORT:
                    st25r95UARTRxCtx.retCode = RFAL_ERR_FRAMING;
                    break; 
                case ST25R95_ERRCODE_62_CRC:   
                    st25r95UARTRxCtx.retCode = RFAL_ERR_CRC;
                    break; 
                case ST25R95_ERRCODE_NOFIELD:
                    st25r95UARTRxCtx.retCode = RFAL_ERR_LINK_LOSS;
                    break;
                default:
                    st25r95UARTRxCtx.retCode = RFAL_ERR_SYSTEM;
                    break;
            }
            if (st25r95UARTRxCtx.retCode != RFAL_ERR_NONE)
            {
            ...
            {
                st25r95UARTRxCtx.retCode = RFAL_ERR_NOMEM;
                st25r95UartFlush();
                break;
            }     
                ...
                else
                {
                    if (st25r95UARTRxCtx.NFCIP1)
                    {
                        ...
                        else
                        {
                            ...
                            st25r95UartFlush();
                            st25r95UARTRxCtx.retCode = RFAL_ERR_PROTO;
                        }
                    }
                    else
                    {
        ...
        case ST25R95_CB_ADDITIONALBYTES: /* Rx flags received */
            ...
            switch (st25r95UARTRxCtx.protocol)
            {
                case (ST25R95_PROTOCOL_ISO15693):
                    st25r95UARTRxCtx.retCode = ST25R95_IS_PROT_ISO15693_COLLISION_ERR(st25r95UARTRxCtx.additionalRespBytes[0]) ? RFAL_ERR_RF_COLLISION : (ST25R95_IS_PROT_ISO15693_CRC_ERR(st25r95UARTRxCtx.additionalRespBytes[0]) ? RFAL_ERR_CRC : st25r95UARTRxCtx.retCode);
                    break;
                case (ST25R95_PROTOCOL_ISO14443A):
                    st25r95UARTRxCtx.retCode = (result == ST25R95_ERRCODE_RESULTSRESIDUAL) ? ((ReturnCode)(RFAL_ERR_INCOMPLETE_BYTE + ((st25r95UARTRxCtx.additionalRespBytes[0] & 0xFU) % 8U))) : (ST25R95_IS_PROT_ISO14443A_COLLISION_ERR(st25r95UARTRxCtx.additionalRespBytes[0]) ? RFAL_ERR_RF_COLLISION : (ST25R95_IS_PROT_ISO14443A_PARITY_ERR(st25r95UARTRxCtx.additionalRespBytes[0]) ? RFAL_ERR_PAR : (ST25R95_IS_PROT_ISO14443A_CRC_ERR(st25r95UARTRxCtx.additionalRespBytes[0]) ? RFAL_ERR_CRC : st25r95UARTRxCtx.retCode)));
                    break;
...
ReturnCode st25r95UARTPollRead(uint32_t timeout)
{
    ...
    return ((st25r95TransmitRxCompleted == false) ? (RFAL_ERR_TIMEOUT) : RFAL_ERR_NONE);
}
...
void st25r95UARTKillIdle(void)
{
    ...
    while ((st25r95PollRead(ST25R95_CONTROL_POLL_NO_TIMEOUT) == RFAL_ERR_TIMEOUT) && (attempt != 0))
    {
        platformDelay(1);
        attempt--;
    }
