...
bool st25r95CheckChipID(void)
{
    ...
    if (st25r95SendCommandTypeAndLen(st25r95CommandIDN, respBuffer, ST25R95_IDN_RESPONSE_BUFLEN) == RFAL_ERR_NONE)
    {
...
{
    ReturnCode retCode;
    uint8_t respBuffer[RFAL_MAX(ST25R95_PROTOCOLSELECT_RESPONSE_BUFLEN, ST25R95_WRREG_RESPONSE_BUFLEN)];
    
    retCode = st25r95SendCommandTypeAndLen(ProtocolSelectCommands[protocol], respBuffer, ST25R95_PROTOCOLSELECT_RESPONSE_BUFLEN);
    if ((retCode == RFAL_ERR_NONE) && (respBuffer[ST25R95_CMD_RESULT_OFFSET] != ST25R95_ERRCODE_NONE))
    {
...
ReturnCode st25r95SetFWT(uint8_t protocol, uint32_t fwt)
{
    ...
    FWT = RFAL_MIN( fwt, ST25R95_FWT_MAX );   /* Limit the FWT to the max supported */
    fwt = FWT;
    PP=0;
    ...
    switch (reg)
    {
        case (ST25R95_REG_ARC_B):
                ...
                (protocol == ST25R95_PROTOCOL_ISO18092))
            {
                ...
                st25r95SendCommandTypeAndLen(WrRegAnalogRegConfigs[protocol], respBuffer, ST25R95_WRREG_RESPONSE_BUFLEN);
                retCode = (respBuffer[ST25R95_CMD_RESULT_OFFSET] == 0) ? RFAL_ERR_NONE : RFAL_ERR_PARAM;
            }
            else
            {
                retCode = RFAL_ERR_WRONG_STATE;
            }
            break;
