...
ReturnCode rfalInitialize(void)
{
    ...
    /* Initialize chip */
    if (st25r95Initialize() != RFAL_ERR_NONE)
    {
...
{
    RFAL_NO_WARNING(txMode);
    RFAL_NO_WARNING(rxMode);
    return;
}
...
ReturnCode rfalSetBitRate(rfalBitRate txBR, rfalBitRate rxBR)
{
    ...
    if ((retCode == RFAL_ERR_NONE) && (gRFAL.protocol != ST25R95_PROTOCOL_FIELDOFF))
    {
...
{
    gRFAL.timings.FDTPoll = RFAL_MIN(FDTPoll, RFAL_ST25R95_GPT_MAX_1FC);
}
...
{
    gRFAL.timings.FDTListen = RFAL_MIN(FDTListen, RFAL_ST25R95_MRT_MAX_1FC);
}
...
{
    gRFAL.timings.GT = RFAL_MIN(GT, RFAL_ST25R95_GT_MAX_1FC);
}
...
ReturnCode rfalFieldOnAndStartGT(void)
{
    ...
    if ((gRFAL.timings.GT != RFAL_TIMING_NONE))
    {
        ...
        rfalTimerStart(gRFAL.tmr.GT, rfalConv1fcToMs(RFAL_MAX((gRFAL.timings.GT),RFAL_ST25R95_GT_MIN_1FC)));
    }
...
ReturnCode rfalTransceiveBlockingRx(void)
{
    ...
    while (((ret = rfalGetTransceiveStatus()) == RFAL_ERR_BUSY) || rfalIsTransceiveInRx());    
...
static void rfalTransceiveRx(void)
{
    ...
    switch (gRFAL.TxRx.state)
    {
        ...
        case RFAL_TXRX_STATE_RX_WAIT_RXE:
            if (st25r95PollRead(ST25R95_CONTROL_POLL_NO_TIMEOUT) == RFAL_ERR_TIMEOUT)
            {
                break;
            }
        ...
        case RFAL_TXRX_STATE_RX_READ_DATA:            
            ...
            if (gRFAL.TxRx.ctx.rxRcvdLen != NULL)
            {
                ...
                /*******************************************************************************/
                if( (retCode >= RFAL_ERR_INCOMPLETE_BYTE_01) && (retCode <= RFAL_ERR_INCOMPLETE_BYTE_07) )
                {
                    (*gRFAL.TxRx.ctx.rxRcvdLen) += (retCode - RFAL_ERR_INCOMPLETE_BYTE);
                    ...
                    retCode = RFAL_ERR_INCOMPLETE_BYTE;
                }
            }
...
static ReturnCode rfalTransceiveRunBlockingTx(void)
{
    ...
    while (((ret = rfalGetTransceiveStatus()) == RFAL_ERR_BUSY) && rfalIsTransceiveInTx());
...
ReturnCode rfalISO14443AGetTransceiveAnticollisionFrameStatus( void )
{
    ...
    /*******************************************************************************/
    if ((*gRFAL.nfcaData.bitsToSend) > 0)
    {
    ...
    {
        (*gRFAL.nfcaData.rxLength)    = rfalConvBytesToBits(gRFAL.RxInformationBytes[1]);
...
ReturnCode rfalGetFeliCaPollStatus( void )
{
    ...
    {
        RFAL_MEMCPY( gRFAL.nfcfData.pollResList, gRFAL.nfcfData.pollResponses, (RFAL_FELICA_POLL_RES_LEN * (uint32_t)RFAL_MIN(gRFAL.nfcfData.pollResListSize, devDetected) ) );
    }
    ...
    return (( (colDetected != 0U) || (devDetected != 0U)) ? RFAL_ERR_NONE : ret);
}
...
ReturnCode rfalListenStart(uint32_t lmMask, const rfalLmConfPA *confA, const rfalLmConfPB *confB, const rfalLmConfPF *confF, uint8_t *rxBuf, uint16_t rxBufLen, uint16_t *rxLen)
{
    ...
    if ((lmMask & RFAL_LM_MASK_NFCA))
    {
        ...
        if (st25r95SetACFilter(confA) != RFAL_ERR_NONE) 
        {
            return (RFAL_ERR_PARAM);
        }
...
static ReturnCode rfalRunListenModeWorker(void)
{
    ...
    st25r95PrepareRx(
     ...
     retCode = st25r95CompleteRx();
     if ((retCode == RFAL_ERR_NONE) && (gRFAL.Lm.rxLen != 0))
     {
...
{
    RFAL_NO_WARNING(info);
    RFAL_NO_WARNING(force);
...
{
    RFAL_NO_WARNING( resI );
    RFAL_NO_WARNING( resQ );
...
{
    RFAL_NO_WARNING( single );
    RFAL_NO_WARNING( rfiox );
